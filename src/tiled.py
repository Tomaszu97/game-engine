...
class TiledManager():
	def load_map(self, filename):
		...
		for obj in root:
			if obj.tag == 'tileset':
				temptree    =   et.parse('../data/'+obj.attrib['source'])
				temproot    =   temptree.getroot()
				for item in temproot:
					if item.tag == 'image':
						tileset         =   pygame.image.load('../data/' +  item.attrib['source'] ).convert_alpha()
...
'''
<objectgroup id="4" name="Warstwa Obiektu 1">
  <object id="2" x="72" y="80" width="136" height="128"/>
  <object id="3" x="192" y="24">
   <point/>
  </object>
  <object id="4" x="248" y="56" width="104" height="88">
   <ellipse/>
  </object>
  <object id="5" x="264" y="224">
   <polygon points="0,0 144,192 -72,192 -112,144 -64,80"/>
  </object>
  <object id="6" x="192" y="280">
   <polygon points="0,0 -136,16 -96,-32"/>
  </object>
 </objectgroup>
'''
